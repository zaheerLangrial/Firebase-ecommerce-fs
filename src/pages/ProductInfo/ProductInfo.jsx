...
const ProductInfo = () => {
  ...
  const getProductInfo = async () => {
    ...
    try {
      ...
      // setProductInfo({ id: tempProduct.id, ...tempProduct.data() });
      setProductInfo({...tempProduct.data() , id: tempProduct.id})
      setLoading(false);
    } catch (error) {
      console.log(error);
      setLoading(false);
  ...
  return (
    <Layout>
      {loading ? (
        <div className="w-full h-[82vh] flex items-center justify-center relative">
          <ScaleLoader className="absolute" color="#d81b60" />
        </div>
      ) : (
